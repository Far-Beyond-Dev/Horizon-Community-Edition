...
fn generate_imports_file(plugin_paths: &[(String, String, String)]) -> std::io::Result<()> {
    ...
    writeln!(file, "use horizon_plugin_api::{{Plugin, LoadedPlugin}};")?;
    ...
    for (name, _, _) in plugin_paths {
        ...
        writeln!(file, "use {}::Plugin_Construct as {}_construct;", name, name)?;
    }
    writeln!(file)?;
    ...
    for (name, _, _) in plugin_paths {
        ...
        writeln!(file, "            instance: <{}::Plugin as {}::Plugin_Construct>::new(plugins.clone(), socket.clone(), players.clone()),", name, name)?;
        writeln!(file, "        }}")?;
        writeln!(file, "    );")?;
    }
